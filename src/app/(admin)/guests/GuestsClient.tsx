...
interface Guest {
  ...
  id_type?:string|null; id_number?:string|null; address?:string|null
  nationality?:string|null; notes?:string|null; created_at:string
  bookings?: Booking[]
}
...
export function GuestsClient({ guests: initialGuests, hotelId }: Props) {
  ...
  const handleAdd = async () => {
    ...
    const { data, error } = await supabase.from("guests").insert({
      ...
      email: form.email||null, id_type: (form.id_type||null) as any,
      id_number: form.id_number||null, address: form.address||null,
      nationality: form.nationality||null, notes: form.notes||null,
    } as any).select("*, bookings(id,status,check_in_date,check_out_date,booking_number)").single()
